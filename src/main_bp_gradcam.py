...
def train(args, model, classifier, dataset_loaders, optimizer, scheduler, device=None, writer=None, model_path=None):
    ...
    for iter_num in range(1, args.max_iter + 1):
        ...
        # TODO: Changed here
        if args.use_bilinear and iter_num <= 800:
            out = classifier(feat_labeled.cuda())
        elif args.use_bilinear:
            out = classifier(bp_out_feat.cuda())
        else:
            out = classifier(feat_labeled.cuda())
        ...
        if args.use_bilinear:
            ...
            with cam_algorithm(model=model_cam,
                ...
                # for weight generated by gradcam
                _, weight = cam(input_tensor=img_labeled_q,
                                target_category=label,
                                aug_smooth=args.aug_smooth,
                                eigen_smooth=args.eigen_smooth)
                weight: torch.Tensor = torch.from_numpy(weight).cuda()  # BxF
